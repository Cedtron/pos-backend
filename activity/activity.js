...
exports.createLog = async (req, res) => {
    ...
    try {
        ...
        db.query(insertSql, [RegNo, username, action, localDate, shop_code], (err, result) => {
            if (err) {
...
exports.getAllLogs = (req, res) => {
    const sql = `SELECT * FROM logs_tb ORDER BY log_id DESC`;
...
exports.updateLog = (req, res) => {
    ...
    db.query(sql, [RegNo, username, action, log_date, shop_code, id], (err, result) => {
        if (err) {
            return res.status(500).send(err);
        }
